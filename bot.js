...
bot.start((ctx) => {
    ...
    if (ADMIN_IDS.includes(tgId) && !user.is_admin) { db.setAdmin(user.id); user = db.findUserByTelegram(tgId); }
...
bot.command('addcredits', (ctx) => {
    const admin = db.findUserByTelegram(String(ctx.from.id));
    if (!admin || !admin.is_admin) return ctx.reply('❌ No tienes permisos de admin.');
    ...
    const bal = db.addCredits(target.id, amount, 'admin_tg', admin.id);
    ctx.reply(`✅ +${amount} créditos → ${targetId}\nBalance: *${bal}*`, { parse_mode: 'Markdown' });
});

bot.command('stats', (ctx) => {
    const admin = db.findUserByTelegram(String(ctx.from.id));
    if (!admin || !admin.is_admin) return ctx.reply('❌ No tienes permisos.');
    const s = db.getStats();
